...
void top_effect(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec4 way = texture2D(u_waveform_tex, 1.0 / u_resolution);
    vec4 spec = texture2D(u_spectrum_tex, 1.0/ u_resolution);

    float wv = way.x;  // Assuming you want the x-component of the vec4
    float sv = spec.x; 
    ...
    vec2 uv = p + 0.1 * vec2(sin(u_time * 0.1), sin(wv * 0.2)) * l * 0.2 * u_time *0.01;
    uv.x *= u_resolution.x / u_resolution.y;
    uv += p / l * (cos(u_time * 0.5) + u_time * 0.008) * abs(cos(l * 15.0 - u_time * 0.1));

    //top color effect
    //vec3 c0 = vec3(0.005 / length(fract(uv)) / l);
    vec3 c1 = vec3(0.005 / length(fract(uv) - 0.5) / l);
    vec3 c2 = vec3(u_time * 0.00001 / length(fract(uv) - 0.5) / l, 
                u_time * 0.00005 / length(fract(uv) - 0.1) / l,
                u_time * 0.000002 / length(fract(uv) - 0.8) / l);
